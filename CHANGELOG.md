...
- 版本更新检测：CLI `--check-update` 查询内部 GitLab 最新 Release，打印当前/最新版本、Release 页链接与资产清单（只检测 + 引导，不自动更新；恒 exit 0，无凭证/网络失败如实提示也不失败）。GUI 顶栏新增「⚙ 设置」子菜单：「启动时自动检查更新」开关（默认开，持久化到 `~/.config/xperf/gui-settings.json`，启动检测失败静默不打扰）+ 手动「检查更新」；发现新版时顶栏出 🆕 徽标，点击查看 Release 说明与资产清单、一键在浏览器打开 Release 页。
...
- 修复采样静默挂死不自愈：adb server 卡死可致转发信道「无数据亦无 EOF」永久半开，图表冻结且前后端均无感知——会话通道增加入向静止看门狗（max(15s, 5×间隔)），超时按断连走自动重连恢复；SSH 隧道空闲期死亡后命令入口（如开始监控）挂起同样自愈（先重建隧道再校验设备）。
- 修复 GUI 长时间监控内存持续增长：图表刻度标签每秒产生新的唯一字符串，WebKit 对渲染/测量过的字符串无淘汰驻留（实测 RSS ~3.7MB/min 线性不收敛）——Y 轴上限档位吸附 + X 轴时间档位对齐后，唯一字符串 90s 450→21，长会话内存恒定。
- 修复「在浏览器打开 Perfetto UI」偶发冻结整个 GUI：该操作曾在主线程同步执行（首次镜像下载可达 ~1 分钟），期间 webview 事件循环全停——改异步执行。
- 修复「开始监控」双击竞态：第二击落在首击请求 pending 窗口内会双开会话并清空已绘图表——前端防重入。
- 修复远程/本机传输切换后设备列表短暂停留旧机队（>15s，设备 tab 与侧栏远程区块随之错位）——切换即刻作废并以新枚举回填设备缓存，设备枚举连续失败留诊断日志。

- 修复 scrcpy 版本过旧时的晦涩失败：镜像/录屏按 scrcpy 4.x 设计与验证，Ubuntu 22.04 apt 的 1.21 等旧版原本只报一条「连接失败」——现在入口处解析 `scrcpy --version`，主版本 <2 直接给出可操作提示（安装 ≥2/推荐 4.x，或用 `XPERF_SCRCPY` 指定路径）。
- 主机工具路径解析补齐 `XPERF_*` 显式覆盖（与 `XPERF_ADB` 同语义：文件存在才生效，缺失回落）：`XPERF_SCRCPY`（scrcpy）、`XPERF_TRACE_PROCESSOR`（trace_processor，另把 `/tmp/trace_processor` 硬编码候选改为系统临时目录 + `/tmp` 双候选，macOS 的 temp_dir 不在 `/tmp`）、`XPERF_CHROME`（Perfetto UI 镜像用的 headless Chrome）。完整解析表见 AGENTS.md；README 双语补可选依赖前置（scrcpy ≥2 / python3 / trace_processor 自动下载）。
...
- 「在浏览器打开 Perfetto UI / 火焰图」按钮防连点重复开标签：进行中禁用（首次镜像下载可达数秒）+ 完成后 5s 冷却（冷却期内点击状态栏提示；600ms 窗口实测拦不住快速连点 burst——曾一次连点打开 3 个标签页），不影响有意重开。
