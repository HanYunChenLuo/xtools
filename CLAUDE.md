...
- **报告段**（trace_analysis.txt + trace_queries.sql 同目录留存）：trace 窗口/boot 基线、包 CPU 总量（单核口径 % 窗口）、包线程 CPU 时间 top15、抢占/调度延迟（thread_state R/R+：唤醒→上核的 runnable 时间）、系统 CPU top 与每核 busy/切换次数（**均须排除 idle：swapper 切片 utid=0 挂 upid=0 无名进程，不排除则空闲机器每核"busy"恒 ~100%、(内核线程) 桶被 idle 淹没 80%+**；排除后 (内核线程) 桶 = 真 kthreads）、CPU 频率（**SS3 GVM 无 cpufreq ftrace 事件，如实标注**，实时值用 --freq）、帧时间线全局统计+最差 5 帧（此平台无进程/图层归属，与 agent 图层 FPS 互补）。
...
- **浏览器一键分析（`reveal_trace_and_open_ui`，core；GUI 的 open_perfetto_ui 命令调用）**：打开 ui.perfetto.dev + 系统文件管理器（Linux 走 dbus `FileManager1.ShowItems` 高亮文件、失败兜底 xdg-open 目录；macOS `open -R`），用户将 trace 拖入浏览器窗口即加载（File API，零网络请求）。**为什么不走本地 HTTP + `?url=` 深链（2026-09-03 实测 Chrome 152 双重拦截）**：① ui.perfetto.dev 自带 CSP `connect-src` 白名单（'self'/localhost:8080/127.0.0.1:9001 等），随机端口与 127.0.0.1 任意端口全被 block（fetch 抛 TypeError，零网络请求）；② 白名单内端口再被 Chrome LNA（Local Network Access）权限拦——公网 https 页面 fetch loopback 需用户授权，深链自动 fetch 无手势被静默拒（带 Access-Control-Allow-Private-Network: true 也过不了）。排查路径：headless Chrome --enable-logging=stderr 看 console（CSP/权限错误都只报在 console，netlog 无请求）。
- **GUI 侧**：主区双 tab（性能指标 / Perfetto 分析），报告不与指标混排；录制期间留在指标页观察实时曲线，done/error 自动切到分析页；分析页顶部「在浏览器分析（拖入 trace）」按钮（recorded 起 enabled，`trace` 事件 payload 附 trace_path）。
