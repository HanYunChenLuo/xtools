...
- **端点**（统一 JSON `{ok, data}` / `{ok:false, error}` + 状态码 400/401/404/503/504）：`GET /api/status`（后端真相：版本/remote/设备列表/每设备采样·深挖·镜像·录屏·logcat·agent_building 状态）；`GET /api/dom?selector=&depth=&max_nodes=`（DOM 子树快照：tag/attrs/text/**bounding rect**/visible，depth≤12、节点≤2000 双上限）；`GET /api/state?serial=`（statusText/activeTab/liveData/peaks/coldStarts/每图表 series 摘要 min-max/**悬停 tooltip**/logcat 尾行）；`GET /api/series?metric=&tail=&at=`（单图表全分辨率尾部 N 点 / at 时刻二分最近点，与悬停取数同口径）；`POST /api/action {op,selector|x,y,…}`（click=mousedown+mouseup+click / input=focus+原生 setter 写值+显式 input+change 事件[根治 AX 时代 set-value 不落、change 不派发两坑] / select / check / hover=mouseover+enter+mousemove 带 clientX,Y[直驱图表悬停] / scroll / key）；`POST /api/eval {expr}`（任意 JS 逃逸舱，async 自动 await，循环引用/函数/DOM 元素安全序列化）。selector 未命中 → 404。
