...
- **浏览器一键打开（`open_in_perfetto_ui`，core；GUI 的 open_perfetto_ui 命令调用）**：起本地单文件 HTTP 服务（127.0.0.1 随机端口，CORS 头 + OPTIONS 预检，进程存活期间有效）→ `xdg-open`/`open` 打开 `https://ui.perfetto.dev/#!/viewer?url=http://127.0.0.1:<port>/<file>` 深链。https 页面 fetch http://127.0.0.1 是 mixed content，但 Chrome/Firefox 均把 loopback 视为 potentially trustworthy 豁免拦截。服务随进程退出终止——进程退出后浏览器内刷新需重新点击按钮。HTTP 服务行为有单测（GET 200+CORS+内容、OPTIONS 204）。
- **GUI 侧**：主区双 tab（性能指标 / Perfetto 分析），报告不与指标混排；录制期间留在指标页观察实时曲线，done/error 自动切到分析页；分析页顶部"在浏览器打开 Perfetto UI 分析"按钮（recorded 起 enabled，`trace` 事件 payload 附 trace_path）。
