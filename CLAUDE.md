...
- **消费方注意**：① 图表悬停 tooltip 经 rAF 合帧呈现，hover 后须轮询 `/api/state` 等 `charts.<m>.hover` 非空，不能同请求即读；② LineChart 悬停内容结构化留存 `_hoverRows/_hoverT`（state 直读，免解析 tooltip HTML）；③ 前端应答错误带 `kind`（not_found→404，bad_params/js_error→400）。
