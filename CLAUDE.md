...
- **断连恢复**：adb logcat EOF → 1s 退避自动重 spawn（与 `reconnect_agent` 语义一致），文件内追加 `# xperf logcat reconnect` 标记行；设备在线但子进程秒死（<3s）连续 3 次判永久性失败（参数错误类），经 `LogcatEvent::Error` 上报后退出（防静默死循环）
...
- **AX 目验教训（2026-09-14）**：① 流式渲染（尤其全机洪泛 ~190 行/s）期间 webkit AX 树会整体剪枝且**粘性不恢复**——按钮交互与内容读取须在启动前的健康窗口一次脚本内完成；② AX `set value` 写文本框须先 `set focused of el to true`，否则值不落 DOM input（曾致按包过滤静默退化为全机抓取——落盘目录是 device-* 而非 pkg 名即为判据）
