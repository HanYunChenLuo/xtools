...
- 本机二进制解析链（`resolve_agent_binary`，64409d4）：`XPERF_AGENT_BIN` env 显式覆盖（空串视同未设置；指向不存在文件报错不回退）→ exe 旁 `agent/xperf-agent`（release tarball 布局）→ 开发检出 `ensure_agent_built()` 兜底（编译期 workspace 有 Cargo.toml 才走）：`target/aarch64-linux-android/release/xperf-agent`（不存在时自动执行 `cargo build -p xperf-agent --target aarch64-linux-android --release`；链接器经 `.cargo/ndk-clang.sh` 探测——macOS/Linux 双平台，NDK **>= 25.1.8937393** 中取最相近，显式 ANDROID_NDK_HOME/ANDROID_NDK_ROOT/NDK_HOME 优先且不过滤，无满足版本时报错列出已发现版本）
