...
- **文本过滤**（CLI `--logcat-regex` / GUI 日志 tab「过滤」输入框，2026-09-15）：设备端 `logcat -e <regex>` 按**消息体**正则过滤（POSIX ERE），与级别/按包过滤叠加；下沉设备端（洪泛场景不把无关行拉过 adb 通道）。**非法正则三平台（A11/A12/A16）均立即 rc=134（regex_error abort）→ 走连续秒死 Error 路径**（CLI 经事件回调透传 stderr，GUI error 事件复位按钮）；落盘文件头与 respawn 标记行均带 `text=` 口径
