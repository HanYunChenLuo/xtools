...
- **会话内 review**：严重 0；一般 1 已修——断连重连 keep-going 闭包原只查 Ctrl-C，限时窗口内设备断开会使采样无限悬挂（`--duration` 有界承诺失效，trace/stack 限时路径预先存在同样问题）；修复 `13f347c`（merge `57250ff`）：闭包加 deadline 判定，真机 `--duration 12` + 第 5s `adb reconnect offline` 强制离线验证到点退出 exit 0。观察项：deadline 起点在 spawn_agent 后（部署不计入窗口——刻意语义，慢部署不应吃掉采样窗口，维持现状）；`--duration`+纯深挖不生效（纯深挖自带边界、无可修对象，help 已注明）；测试模块带入的 EOF 尾部空行**已顺手修复**（随 13f347c，尾字节验证干净）
