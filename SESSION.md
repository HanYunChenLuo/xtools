...
| （本条目补 hash） | cargo doc 编译错误 + 大量 warning 修复（用户实测抓出）：完整 `cargo doc` 暴露三类 rustdoc 默认 lint 问题——①15 处 doc 裸尖括号（`/proc/<pid>/io` 等被 rustdoc 当未闭合 HTML 标签；xperformance 的 `#![deny(warnings)]` 将其升级为 **error 导致 cargo doc 失败**）→ 路径/参数包反引号（code span 不解析 HTML）②trace.rs 模块级 doc 裸 URL → markdown 链接③10 处日志样例 `[GPU0]` 被当 intra-doc 链接 → code block/code span 包裹。**教训（已入 CLAUDE.md Commands）**：上轮宣称"cargo doc 检查过"实际只跑了 `rustdoc -W missing_docs` 单 lint——默认 lint 集（invalid_html_tags/bare_urls/broken_intra_doc_links）根本没跑，验证不完整；以后必须跑完整 `cargo doc` 且 warning 归零。修后：cargo doc 零 warning 零 error、69 测试全绿、clippy 零警告、4 crate missing_docs 归零 |
