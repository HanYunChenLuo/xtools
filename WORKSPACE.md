...
- SS2MAX gpubusy 计数器恒 `0 0`（2026-09-03 实测 30 次采样全零，total_time 停走 → kgsl busy 通道无事件；gpuclk 正常 427MHz）——数据源限制，新旧 agent 行为一致。**2026-09-07 再观察：kgsl busy 通道有事件但 busy% 可 >100%（GpuUpdate busy=1662% 实测，gpubusy 计数器 total/busy 相位错乱的产物）——同属 kgsl 计数器停走类数据源限制，值不可信，需真 COLD 场景另行核实**
