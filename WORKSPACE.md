...
> 最后更新：2026-09-23（四）：I 节「版本更新检测（GitLab Release）」完成合入（merge `54cd3e1`）
...
- [x] ~~**版本更新检测（GitLab Release）**~~（**已完成**，2026-09-23，`feature/update-check` 合 main，merge `54cd3e1`；子提交 `508573c` core + `5c60361` CLI + `36e33dd` GUI + `dd30970` docs + `e9b4d45` diag）：core `gitlab.rs`（从 feedback.rs 提取公共 GitLab 访问：resolve_auth 凭证链/gitlab_api/project_id/project_web_url/auth_hint）+ `update.rs`（fetch_latest_release=GET releases 列表 semver 选最新 5s 超时，is_newer 纯函数非法格式如实 None）；CLI `--check-update`（恒 exit 0，无凭证如实指引）；GUI 顶栏「⚙ 设置」子菜单（启动自动检查开关默认开，`~/.config/xperf/gui-settings.json` 原子写持久化）+ 手动检查 + 🆕 徽标 → 模态（说明纯文本/资产清单/open_url 仅放行 http(s)）。启动检测走前端 init 发起（避开后端事件早丢坑），失败静默+diag。单测 core +7/GUI +1（设置 roundtrip 路径注入免 env 突变）。真机验证：CLI 对 v0.3.2「已是最新」exit 0、无凭证（隔离 HOME）exit 0；GUI 调试 API 全链（菜单开合/手动检查状态栏结论/开关持久化双向/注入假 release 徽标→模态内容→关闭/open_url scheme 拒绝/设置关→重启无检测、开→diag 一行）
  - **原设计决策留档**：只检测+引导不做自动更新/自替换；Release 描述模态内按纯文本展示不渲染 markdown；「有新版」路径真机天然场景待 v0.3.3 发布后自然覆盖（单测已锁比较逻辑）
