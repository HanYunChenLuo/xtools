...
> 最后更新：2026-09-19：I 节新增候补「GUI 可编程操控/状态读取接口」（agent 自助目验，免截图+模拟点击，新会话执行）；当日已完成：D 节 GUI 折线图悬停精确读数（4eeaad6 + review 修复 15bef72 + 残余清零 8b43816）
...
- [ ] **GUI 可编程操控/状态读取接口（agent 自助目验，免截图+模拟点击）**（2026-09-19 悬停读数会话痛点驱动）：当前验证 GUI 行为只能靠 CGEvent 鼠标模拟 + screencapture 截图 + 按布局估算坐标（脆弱，布局变即失效），且 WKWebView AX 树冻结于启动早期快照（新插入内容不可达，见 CLAUDE.md AX 教训）。目标：GUI 提供 **env 开启的本地调试通道**（如 `XPERF_GUI_DEBUG=1` 起 127.0.0.1 HTTP/Unix socket，端口写 stderr/文件），两类能力：①**读状态**——设备列表/活跃 serial、采样状态、状态栏文本、实时数值面板、图表 series 摘要（末 N 点或指定时刻值，含悬停 tooltip 内容）、logcat 视图行；②**注入操作**——开始/停止、指标勾选、切 tab/设备、按钮点击、输入框填值、hover 图表坐标。形态建议：后端调试端口收请求 → emit Tauri 事件 → 前端 debug hook（如 `window.__xperfDebug`）执行并以 invoke 回传结果；默认关闭、仅 loopback。用途：GUI 改动的自动化目验回归（替代截图比对），也服务 coding agent 自助验证。
