...
> 最后更新：2026-09-23（三）：独立 review follow-up 全修合入（5 一般 + 7 观察，`eb83624`）；I 节新增「版本更新检测（GitLab Release）」排期（设计决策已拍板，新会话实施）
...
- [x] ~~**scrcpy 版本地板提示**~~（**已完成**，2026-09-23 路径审计顺手修，`86ebf1c`）：`find_scrcpy` 增 `XPERF_SCRCPY` env 覆盖；`spawn_scrcpy` 入口解析 `--version` 主版本，<2 直接 bail 可操作提示（`SCRCPY_MIN_MAJOR=2`；`--version` 起不来不挡，按原路径让真实失败上报）。README 双语/AGENTS.md 前置条件同步。真机验证双场：hppc 假脚本 1.21/4.9.9（门槛拦截 vs 过门槛）+ kong 真 1.21（CI bullseye 产物，错误文案含 `/usr/bin/scrcpy` 与升级指引，rc=1）。
- [ ] **版本更新检测（GitLab Release）**（2026-09-23 排期，新会话实施）
  - **目的**：GitLab 发布新版本后，CLI/GUI 能检测到「有新版」并引导升级——用户现在只能靠人肉刷 Release 页。
  - **已拍板的设计决策**（新会话直接按此实施，勿再发散）：
    1. **只检测 + 引导，不做自动更新/自替换**：AppImage/DMG/tar 三形态自动替换复杂且危险（.app 换文件破签名）；输出 Release 页链接 + 资产清单（package registry 链接带登录态浏览器可直接下），升级动作留给用户。
    2. **core 新模块 `update.rs`**：`ReleaseInfo { tag, released_at, url, description, assets }`；`fetch_latest_release()` 走 `GET /api/v4/projects/39859/releases`（reqwest blocking + rustls，与 feedback.rs 同款；超时 5s；项目私有 → 复用 `feedback::resolve_auth` 凭证链——顺带把 resolve_auth 从 feedback.rs 提为 pub 共享或挪独立小模块）；`is_newer(latest_tag, current)` 纯函数（strip `v` 前缀，semver 比较——tag 格式已被 validate:tag 锁定 vX.Y.Z；Cargo.lock 已有 semver 传递依赖，显式加无树负担）。
    3. **CLI `--check-update`**（无需 `--package`，与 `--clean-cache` 同类）：打印 当前版本 / 最新版本 / 是否有更新 / Release URL / 资产名清单；无凭证时如实提示配置方法（`GITLAB_TOKEN` / `--gitlab-login`）；**退出码恒 0**（检测完成即成功，「有新版」不是错误——脚本/agent 不因此失败）；不采样不建会话目录。
    4. **GUI**：启动后台线程检测一次（失败静默，不打扰）；有新版时顶栏徽标「新版本 vX.Y.Z」（点击弹模态：Release 说明（描述即 CHANGELOG 章节）+ 资产清单 + 「在浏览器打开 Release 页」按钮——opener 模式同反馈浮层）；顶栏另加手动「检查更新」入口（无新版也给状态栏确认）。
    5. **不做**启动强制网络、不做定时轮询、不代下载资产（registry 链接需登录态，列名即可）。
  - **实施步骤**：① core `update.rs` + resolve_auth 提取共享 + 单测（tag 解析/semver 比较/Release JSON 解析——脱敏样例内嵌）+ `#[ignore]` 真机集成测试（复用 XPERF_IT_* 环境注入套路，需凭证时 SKIP 语义）② CLI flag + main 分支（在 `--clean-cache` 类似位置）③ GUI 命令 `check_update` + 顶栏 UI + 启动后台检测 ④ 文档：README 双语功能表、AGENTS.md（agent 可 `--check-update`）、CLAUDE.md 新模块节、CHANGELOG。
  - **验收**：单测全绿 + clippy/doc 零警告；真机（hppc，PAT 经 `~/.git-credentials` 取）CLI `--check-update` 对 v0.3.2 出「已是最新」、篡改本地版本比较逻辑反向验证「有新版」路径（或等 v0.3.3 后天然有新版）；GUI（hppc 直编）徽标/模态/手动检查全链；`--check-update` 无凭证路径提示正确；SSH `--remote` 模式下 CLI 可用（检测走 host 网络本就无关 adb）。
  - **注意**：检测功能发版时自身版本是"当前"——首次发布后才有「有新版」可测，验收用反向比较逻辑兜底；Release 描述里是 Markdown，模态里按纯文本展示即可（不引入渲染）。
...
- [x] ~~**K1：hppc GUI `--remote kong`（覆盖「Linux 上的 SSH 远程模式」）**~~（**已完成**，2026-09-22 深夜，kong 车机通电后执行；顺带首次覆盖「SSH 远程 + SS4 桥接」组合）：hppc 直编 GUI（含 A 节修复的 main 二进制）`--remote kong` + Xvfb `:99`，套件在 hppc 同机跑（`XPERF_TEST_SSH/XPERF_TEST_SSH_UI=kong`、`XPERF_TEST_EXTRA_SERIALS=localhost:5559`、`XPERF_TEST_LOCAL_SERIALS=6eb792dfb0f,d1f39648c1f`）。**结果 g1 29/29、g2 22/22、g3 15/15、g4 16/16、g5 14/14、g6 10/10、g8 3/3 全绿**；kong 侧枚举出 SS2PRO `ac889a71b1f`(A11) + 桥接 `localhost:5559`(HU_SS4/A16) + 网关 `42087266b1f`（is_gateway=true 被前端过滤）——桥接在 hop#1 上自动 bootstrap，SSH 隧道重建用例（杀 master→rebuild→恢复采样）首次在 Linux 上真跑通过。
  - **本项抓出的真缺陷（已修）**：「切回本机」后 `/api/status` 设备列表停在远程机队 >15s（前端 tab/侧栏随之错位）——设备列表读监视器 3s 快照，切换瞬间的枚举失败被 `Err(_) => continue` 静默吞掉，旧快照无限期充当后端真相。修=传输切换即刻作废并用新枚举 seed 缓存 + 连续失败经 `utils::diag` 留痕（`dbef7fb`）。
  - **判据侧修正（环境假设，非缺陷）**：g1 切回本机改轮询；g4 逐设备建立「装了包 + 真的在跑」前置（起不来如实 SKIP）、隧道 socket 前缀取 `remote.host`；g3 trace 产物阈值 1MB→50KB（SS2PRO/A11 5s 实测 476KB）。见 `53bafee`/`9b8a17b`/`9a298a9`。
  - 机队差异记录：kong 的 SS2PRO 装 `hellotriangle`、SS4 装 `gltf viewer`（无共同测试包）→ 为覆盖并行语义把 `example/apk` 的 gltf viewer 装到 SS2PRO（A11 接受，冷启动 1227ms）；hellotriangle 装到 SS4 未成功（`adb install` 经桥接 TCP 挂住超时）——记为已知边界，非工具问题。
  - 原计划步骤（留档）
...
- [x] ~~**K2：kong AppImage v0.3.1 本地模式（发布二进制等效性）**~~（**已完成**，2026-09-22 晚，**执行环境改为 hppc**：kong 无设备挂载，而 hppc 挂着 SS3+SS2MAX+SS4 且系统更新——测的是同一份发布产物）
  ...
  - **覆盖判读（如实记录）**：① ~~22.04 未覆盖~~ → **已由 K2b 补齐**（kong AppImage 本地模式，2026-09-22 深夜）；本轮 hppc=24.04 多验了沙箱 hook 分支，两机合成完整覆盖；② v0.3.1 不含 9-21/9-22 修复，本轮相关用例通过不可当回归证据（g4「start 双击仅一次」是防重入修复缺失下的偶发通过；g3「连点不炸」判据只查存活不查标签页数）；③ 录屏/镜像 SKIP（hppc 未装 scrcpy）、SSH 隧道重建 SKIP（本机模式）
  ...
  - **后续**：v0.3.2 发版后按 K2b 同法在 22.04 + 24.04 两机各复跑一轮（含 K1 的 Linux-SSH 模式），拿「含全部修复」的发布验证
- [x] ~~**K2b：kong AppImage 本地模式（Ubuntu 22.04 覆盖补齐）**~~（**已完成**，2026-09-22 深夜，车机通电后）：跑的是**含 A 节修复的 CI 产物 AppImage**（pipeline #1420946 gui:linux artifact，114MB，scp hppc→kong），`xvfb-run -a` 本地模式，套件在 kong 同机跑（`XPERF_IT_PACKAGE=com.google.android.filament.gltf`、`XPERF_TEST_LOCAL_SERIALS=ac889a71b1f,localhost:5559`）。**结果 g1 2/2+SKIP(SSH 小节)、g2 22/22、g3 14/14+SKIP2、g4 16/16+SKIP(隧道，本机模式无)、g5 14/14、g6 10/10、g8 3/3 → 总计 FAIL 0**。
  - **发布产物闭环**：22.04（无 userns 限制）上 WebKit 沙箱保留、AppRun 条件 hook 不触发（与 hppc 24.04 相反分支）✓；**火焰图在第二台非构建机上 fresh+reuse 双路径产出 3.6MB HTML** ✓（A 节修复的跨机验证，同时也证明了 Linux AppImage 侧的 bundle 脚本集生效）。
  - **三条环境门槛如实 SKIP（判据已改，`9a298a9`）**：① kong 的 apt scrcpy 是 **1.21**，本工具按 scrcpy 4.x 设计（`--tunnel-port`/隧道形态）→ 主版本 <2 直接 SKIP 并写明原因；② 离线机拿不到 `trace_processor`（无本地缓存 + get.perfetto.dev 不可达）→ trace 分析判据 SKIP（产品如实报「分析失败 + trace 已保存」是对的），产物判据保留；③ 慢机火焰图首轮渲染分钟级，判据改「产物 HTML 出现即通过」双轨。
  - **候补（产品侧，值得做）**：scrcpy 版本过低时目前只有失败弹窗——`find_scrcpy` 之后加一条版本探测，提示「需 scrcpy ≥2（推荐 4.x），当前 1.21」；见 I 节。
