...
- [x] **Filament 源码对照与优化验证（2026-09-15 完成，真机回归全通）**：根因锁定为 **face_lib 3DGS 排序管线每帧 15 次 `flush_and_wait`**（radix sort 4 pass × 3 kernel + init + calc_dist + calc_view；radix pass 循环内调用，静态 6 调用点 = `GpuSorter::dispatch_gpu`×4 + `sort_points` + `calc_view_data_gpu_full`），两版该应用行为一致，但 1.74 后端 `VulkanDriver::finish()` = flush + **vkQueueWaitIdle** + waitForFences + readPixels（1.38 仅 flush），且 1.74 每次 submit 用**新 semaphore + 链式 wait**（`mLastSubmit`，upstream #8246 引入）→ Adreno 每 submit 做 kgsl syncobj 创建/销毁 + FD 开关；1.38 固定 45 槽 semaphore/fence 复用无此churn。逐 submit 成本 1.74 ≈ 4-5× 1.38（同协议 simpleperf 对照：flush 子树 16.40% vs 4.85%、vkQueueSubmit 13.29% vs 2.71%，另 1.74 独有 getRecording+vkBeginCommandBuffer 8.36%/7.28%——1.38 全零）。
  ...
  - **产物**：补丁 `/tmp/facedemo/batched-sort-1.74.patch`（hppc，311 行，作用于 mindui-1.74 `face_lib/.../gaussian_splat/{sorting,renderer}.rs`，原文件备份为同目录 `*.bak-batching`）；优化 APK `/tmp/facedemo/v1.74.1-batching.apk`（当前已装 SS4；回退 = `adb install -r /tmp/facedemo/v1.74.1.apk`）。构建命令：`cargo build --target aarch64-linux-android -p facedemo --release --lib --features "filament_source backend_vulkan" --target-dir target/facedemo`（rustup nightly-2025-02-14 + NDK env；APK 重打包用 zip STORED + zipalign -p 4096 + platform.jks 12345678/chehejia）。
  - **残余候选（未做，供后续）**：1.74 后端 `finish()` 去 vkQueueWaitIdle（对齐 1.38 语义，flushAndWait 的 FFence 已兜底）可再省残留调用方成本；`mLastSubmit` 链式 wait + 每 submit 新 semaphore 是 Adreno syncobj churn 之源，可按需信号化（仅 present 前最后一次 submit 带 signal）——两者影响面为整个 fork 后端，建议在 filament 仓库单独评审。每帧 SurfaceRenderer drop→endFrame→present 的 fence FD churn（fdsan close/gsl_syncobj，两版共有）是现占比最高的剩余路径。
