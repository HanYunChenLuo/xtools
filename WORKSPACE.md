...
- [x] ~~**软件发布打包（Linux + macOS，结合 GitLab CI）**~~（已完成，2026-09-15，合并提交 `56e5f6b`、`db4d26e`、`df1fae4`、`e25bb9d`）：四 crate 版本统一 `0.2.0`；GitLab CI 使用内部 Artifactory 镜像、Aliyun apt/crates 镜像，tag 流水线 validate→test→build→release；Linux x86_64 包含 CLI + 预编译 Android agent；NDK r25b（507MB）与 Android rust-std（26MB）vendor 到 package registry，build 实测 550s→106s；无 macOS runner，`scripts/release-macos.sh` 本机生成 arm64/x86_64 包上传同一 Release；`CHANGELOG.md` 驱动 Release 描述；GitHub workflow 删除。`v0.2.0` Release 已验证 Linux/macOS 双架构三资产可下载，资产链接统一使用 package registry API 路径（`/-/package_files/<id>` 在本 GitLab 实例 404）。
