...
> 最后更新：2026-09-15 下午：**J 节完成——1.74 CPU 增量根因 = face_lib 每帧 15 次 flush_and_wait 放大 1.74 后端 submit/finish 重路径；应用侧批量提交优化落地并真机回归（进程 16.89→9.70%、XFW:Main 12.21→5.08%、FPS/GPU 不变），补丁与 APK 在 hppc /tmp/facedemo/**。I 节仅剩「命令行输入」
...
- [x] ~~**filament 1.74 CPU 升高归因**~~（**已完成**，2026-09-15，真机 SS4 `--remote hppc` 全程实测）：场景 = `毛绒 3DGS 调试`（FurBoy3dgsActivity2）+ niuzai 3DGS 模型 + awaken2 循环播放（用户指正场景入口；FaceActivity/PIXS 均非目标）。**核心结论**：
  ...
  - **主线 1.38 的 awaken2 专属 Vulkan 管线重建病理**：CPU 的 50.6% 持续花在 `VulkanPipelineCache::createPipeline` → `vkCreateGraphicsPipelines` → Adreno `libllvm-qgl` 着色器编译（总 CPU 15.4%，10+ 分钟无衰减；换 `broadcast_error` 后 createPipeline=0%）。**机制（反汇编实锤）**：1.38 的 `bindPipeline` 以 336 字节渲染状态快照为缓存 key（MurmurHash3 + 逐字节 memcmp），`bindRenderPass` 把 **VkRenderPass 指针**、`bindVertexArray` 把顶点缓冲句柄直接写入 key（按句柄身份而非兼容性）——awaken2 每帧点排序/缓冲重建带来句柄变化 → 每帧 miss → 全量编译。1.74 Vulkan 后端已重构（函数族完全不同）不受影响
  - **实测与文档的差异（已解释）**：v1.74.1 实测 12.0% 吻合文档 11.6%；主线 `awaken2` 实测 15.4% 是额外管线编译病理，而 `broadcast_error` 下主线渲染线程 7.26% 与文档 7.3% 对齐，说明文档测量状态未触发该 1.38 专属病理。具体文档动画仍待后续源码/记录确认，但不再是工具口径或固件差异的硬 blocker
...
- [x] **Filament 源码对照与优化验证（2026-09-15 完成，真机回归全通）**：根因锁定为 **face_lib 3DGS 排序管线每帧 15 次 `flush_and_wait`**（radix sort 4 pass × 3 kernel + init + calc_dist + calc_view；radix pass 循环内调用，静态 6 调用点 = `GpuSorter::dispatch_gpu`×4 + `sort_points` + `calc_view_data_gpu_full`），两版该应用行为一致，但 1.74 后端 `VulkanDriver::finish()` = flush + **vkQueueWaitIdle** + waitForFences + readPixels（1.38 仅 flush），且 1.74 每次 submit 用**新 semaphore + 链式 wait**（`mLastSubmit`，upstream #8246 引入）→ Adreno 每 submit 做 kgsl syncobj 创建/销毁 + FD 开关；1.38 固定 45 槽 semaphore/fence 复用无此churn。逐 submit 成本 1.74 ≈ 4-5× 1.38（同协议 simpleperf 对照：flush 子树 16.40% vs 4.85%、vkQueueSubmit 13.29% vs 2.71%，另 1.74 独有 getRecording+vkBeginCommandBuffer 8.36%/7.28%——1.38 全零）。
  - **优化（应用侧 face_lib，未动 filament 源码）**：排序管线批量提交——upsweep/scan/downsweep 每 pass 独立 MaterialInstance（共 4 份，消除 host 即时 descriptor 更新与在飞 GPU 读的竞争；UBO 参数本就走 staging copy 有 cmdbuf 保序），删除全部 15 次 flush_and_wait，dispatch_gpu 末尾与 calc_view 末尾各留一次 `flush()`（保「compute 先于 render pass」语义）。正确性依据：后端 dispatchCompute 每 dispatch 后已插 compute→* barrier（1.74 的 Lixiang fix）。
  - **真机回归（SS4 localhost:5559 经 --remote hppc，broadcast_error 循环 60s）**：进程 CPU 16.89% → **9.70%**，XFW:Main 12.21% → **5.08%**（比主线 1.38 的 11.69%/7.15% 还低——1.38 同样付 15 次 submit+fence 等待），FPS 44.82 不变，GPU 进程 busy 10.67%（基线 11.94% 同量级），RSS 192MB。awaken2 对照：10.24%/5.15%/44.82，无管线编译病理。simpleperf 证据（children，进程口径）：flush 16.40→4.12%、submit 16.22→4.10%、vkQueueSubmit 13.29→1.72%、updateBufferObjectCommon 12.60→3.98%、dispatchCompute 7.60→4.13%、getRecording 8.36→1.36%、vkBeginCommandBuffer 7.28→1.30%，finish/vkQueueWaitIdle/syncobj create+destroy 消失。截图目验 niuzai 渲染正确，logcat 无 Vulkan/排序错误。
  - **产物**：补丁 `/tmp/facedemo/batched-sort-1.74.patch`（hppc，561 行，含 B1 静态化，作用于 mindui-1.74 `face_lib/.../gaussian_splat/{sorting,renderer}.rs`，原文件备份为同目录 `*.bak-batching`）；优化 APK `/tmp/facedemo/v1.74.1-batching-b1.apk`（当前已装 SS4；纯批量提交版为 `v1.74.1-batching.apk`；回退 = `adb install -r /tmp/facedemo/v1.74.1.apk`）。构建命令：`cargo build --target aarch64-linux-android -p facedemo --release --lib --features "filament_source backend_vulkan" --target-dir target/facedemo`（rustup nightly-2025-02-14 + NDK env；APK 重打包用 zip STORED + zipalign -p 4096 + platform.jks 12345678/chehejia，脚本 `/tmp/repack_apk.sh`）。
  - **B1 参数/绑定静态化（2026-09-15 下午叠加，真机回归通过）**：排序每个 pass-MI 的参数（radixShift/numKeys/threadBlocks）与 SSBO 绑定逐帧恒定 → 从每帧 dispatch 前设置挪到 `GpuSorter::bind_static()` 一次设置；`static_bind_key`（count + 6 个 buffer 的 Rc 指针）守卫资源重建/换模型时自动重绑。四个 dispatch_* 瘦身成纯 dispatch。真机（broadcast_error 60s）：进程 CPU 9.70% → **9.17%**，XFW:Main 5.08% → **4.64%**，FPS 44.85 / GPU 10.66% 不变，渲染目验正确。消除量 = 每帧 ~100 次 vkUpdateDescriptorSets + ~12 次 UBO staging copy。
  - **残余候选（未做，供后续）**：①B2 隔帧排序（`sort_nth_frame` 字段已存在，默认 1；改 2/3 或按相机位移触发可直接减半排序全链路，需目验确认无深度闪烁）；②B3 shader 层重构（Onesweep 单 pass 排序或 16-bit key，dispatch 13→4/7，收益在 GPU 帧时间，改动大）；③后端 `finish()` 去 vkQueueWaitIdle / `mLastSubmit` 链按需信号化——热路径已不走 finish，收益微小，仅在 filament 仓单独评审时有意义；④每帧 SurfaceRenderer drop→endFrame→present 的 fence FD churn（fdsan close/gsl_syncobj，两版共有）是现占比最高的剩余路径。
