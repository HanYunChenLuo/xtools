...
- GUI：9 张折线图 + 实时数值面板 + Top 线程 + 峰值 + 间隔档位下拉 + 实际周期标注 + 勾选即时生效（自动重启会话）+ 打点竖线 + Perfetto 分析（秒数+按钮 → 独立 tab 报告 + 一键浏览器打开 ui.perfetto.dev）
- agent 部署：自动尝试 adb root（IO 等需 root）；src 树内任一 .rs mtime 变化自动重建
- 测试：**68 全绿**（agent 24 个：解析 + watchdog_step 决策；trace 7 个：config/SQL/解析/报告/HTTP 服务），clippy 零警告
