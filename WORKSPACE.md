...
> 最后更新：2026-09-15 下午：facedemo 性能调试任务完成并清理出本仓库（与本仓库无关，产物在 hppc `/tmp/facedemo/`）；I 节仅剩「命令行输入」
...
- [ ] **logcat 文本过滤**（2026-09-15 新增）：现有按包过滤（--uid/--pid）之外，增加按文本内容过滤（关键字/正则，形态待定：设备端 `logcat -e` 正则 vs host 侧过滤——文本过滤吞吐敏感，洪泛场景宜尽量下沉设备端）。要求：CLI 参数 + GUI 日志 tab 输入框；与级别/按包过滤叠加生效；口径变更走已有热切换路径（restart 同文件续写）。
