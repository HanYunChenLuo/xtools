...
> 最后更新：2026-09-22（深夜·二）：**K 节三项全部收官**——K1 ✅（hppc GUI --remote kong：Linux SSH 远程模式 + SSH/桥接组合全绿，顺带修「传输切换后设备列表停留旧机队」缺陷）、K2 ✅（v0.3.1 AppImage @24.04，沙箱 hook 闭环）+ K2b ✅（CI 产物 AppImage @22.04 kong 本地模式，FAIL 0，火焰图修复跨机复验）、K3 ✅（issue #5 关闭）；A 节火焰图发布缺陷两层根治亦已闭环
...
- [ ] **scrcpy 版本地板提示**（2026-09-22 K2b 实测）：Ubuntu 22.04 apt 只有 scrcpy 1.21，本工具的镜像/录屏按 scrcpy 4.x 设计（`--tunnel-port` 双钉同号、`localabstract:scrcpy-*` 规则清扫），1.21 起不来时用户只看到一条失败弹窗。修法=`find_scrcpy` 后跑一次 `--version` 解析主版本，<2 直接返回可读错误（「需 scrcpy ≥2，推荐 4.x；当前 1.21」），并写进 README 前置条件。
